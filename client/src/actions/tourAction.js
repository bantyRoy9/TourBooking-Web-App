...
export const getAllTour = (keyword=' ',currentPage= 1,category, sortBy='')=> async(dispatch)=>{
    try{
       ...
       // console.log(keyword);
        let link = `${URL}/api2/v1/tours?${keyword}&page=${currentPage}&sort=${sortBy}`

        if(category){
            link = `${URL}/api2/v1/tours?search=${keyword}&category=${category}`
        }
...
export const getTourDetail = (id) => async(dispatch)=>{
    try{
        ...
        const {data} = await axios.get(`${URL}/api2/v1/tours/${id}`)
